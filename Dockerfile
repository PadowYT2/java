...
RUN apt-get update -y \
    ...
    && curl --retry 3 -Lfso /tmp/graalvm.tar.gz https://download.oracle.com/graalvm/${JAVA_VERSION}/latest/graalvm-jdk-${JAVA_VERSION}_linux-_$(if [ "$PLATFORM" eq "arm64" ]; then echo "aarch64"; else echo "x64"; fi)bin.tar.gz \
